	...
	for name, proposedValue := range vs {
		...
		case config.VariableTypeString:
			...
			case string:
				continue
			default:
				errs = append(errs, fmt.Errorf("XXXX  variable %s should be type %s, got %s",
		...
		case config.VariableTypeMap:
			...
			case map[string]interface{}:
				continue
			default:
				errs = append(errs, fmt.Errorf("YYYY variable %s should be type %s, got %s",
					name, declaredType.Printable(), hclTypeName(proposedValue)))
			}
		case config.VariableTypeList:
			...
			case []interface{}:
				continue
			default:
				errs = append(errs, fmt.Errorf("ZZZZ variable %s should be type %s, got %s",
					name, declaredType.Printable(), hclTypeName(proposedValue)))
			}
		default:
			errs = append(errs, fmt.Errorf("MMMMM variable %s should be type %s, got %s",
				name, declaredType.Printable(), hclTypeName(proposedValue)))
		}
	}
